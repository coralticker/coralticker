// The ONLY writer of email_signups.confirmed_at: the notifier filters
// confirmed_at IS NOT NULL AND unsubscribed_at IS NULL, so until this route
// runs, every captured row is DOI-pending and the digest reaches nobody.
//
...
// applied to /unsubscribe only. Known minor DOI weakness, accepted.
...
export async function GET(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    // The link stays valid — a re-click retries the write — so we still land the
    // confirmed page rather than dead-ending the click on a transient DB error.
