"""Apply migration 0060 — CTK-148: Reef Regeneration vendors row (id=34).

INSERT ... ON CONFLICT (slug) DO NOTHING + a plain MAX(id) setval bump (fleet
idiom per 0059's SQL body; not a GREATEST guard) — idempotent, re-runnable, no
DROP, no destructive write. Seeds the data-side prerequisite the Reef
Regeneration scraper reads at stage 1 (Config) via db.fetch_vendor.

Verification:
  - vendors row present for slug='reefregeneration'
  - id=34, platform='shopify', scrape_method='products_json', active=true
  - display_name="Reef Regeneration", base_url='https://reefregeneration.com'
"""
...
MIGRATION_PATH = (
    ...
    / "supabase"
    / "migrations"
    / "0060_add_reefregeneration_vendor.sql"
)

EXPECTED_SLUG = "reefregeneration"
EXPECTED = {
    "id": 34,
    "display_name": "Reef Regeneration",
